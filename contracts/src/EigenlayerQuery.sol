...
contract EigenlayerQuery is CoprocessorAdapter {
    ...
    mapping(address => mapping(address => mapping(bytes32 => uint256))) public operatorBalances;
    ...
    function handleNotice(bytes32 payloadHash, bytes memory notice) internal override {
        ...
        (bytes32 blockHash, address operator, address erc20, uint256 balance) = abi.decode(
            notice,
            (bytes32, address, address, uint256 )
        );

        delete queriesInProgress[payloadHash];
        operatorBalances[operator][erc20][blockHash] = balance;
    }
}
